    ...
    async init() {
        ...
        try {
            ...
            const local = window.location.protocol === 'file:'
                ? ' La page est ouverte en file:// : le navigateur bloque la lecture des fichiers locaux, servez le site en HTTP.'
                : ' Vérifiez que data/transcodification.json est bien déployé.';
            if (list) {
                list.innerHTML = `<p class="transco-error">Impossible de charger le référentiel des cas d’usage (${err.message}).${local}</p>`;
            }
            return;
        }
        this.renderMeta();
